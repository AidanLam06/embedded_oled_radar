...
#define MIN_DUTY (409) // floor(0.001/0.02 * 2^14)
#define MAX_DUTY (1966) // floor(0.002/0.02 * 2^14)
#define STEP (9)
...
// stdbool.h refused to work after trying many different solutions
#define true (1)
#define false (0)
...
void write_to_oled_task(void *args) {
    ...
    for (;;) {
        ...
        loop_timer.end = esp_timer_get_time();
        cycle_time = loop_timer.end - loop_timer.start;
        vTaskDelay((DESIRED_LOOP_DURATION_MS - cycle_time)/ portTICK_PERIOD_MS); // fills remainder of desired loop duration using this calculation based on the cycle duration (sensor->oled). doesn't consider servo step because the lion doesn't concern himself with servo rotor stalling and assumes constant step time
        xSemaphoreGive(oled_to_servo);
        // vTaskDelayUntil(&prev_refresh, refresh);
    }
}

void startup_ritual_task(void *args) {
    ESP_LOGI(tag, "startup_ritual");
    xSemaphoreGive(servo_to_ultra);
    vTaskDelete(NULL);
}
